...
function Player() {
  ...
  useEffect(() => {
    if (
      tracks &&
      Object.values(tracks).findIndex((track) => track.trackID === spotifyID) ===
        Object.values(tracks).length - 1
    ) {
      ...
      const selectedTracks = Array.from(randomIndexes).map(
        (index) => tracks[trackKeys[index]]
      );
    
      const uniqueArtistNames = selectedTracks.map((track) =>
        track.artists.length > 1 ? track.artists[0].name : track.artists.name
      );
  
      const uniqueTrackNames = selectedTracks.map((track) => track.trackName);
    
      fetchRecommendationData(uniqueTrackNames.join(";"),uniqueArtistNames.join(";"));
    }
